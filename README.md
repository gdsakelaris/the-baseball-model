...
| `Tools/` | The manually-run toolkit, numbered in game-day order: `1_get_todays_games.py` (today's lineups/weather), `2_scrape_odds.py` (real closing lines, near game time), `3_gui.py` (the Tkinter app), `4_grade_results.py` (grade finished slates; `--all` grades every workbook + the accumulated forward record), `5_prop_rankings.py` (the per-market quality board → `PROP_RANKINGS.xlsx`: Scores, tiers, blue-mark depths; renamed from `prop_rankings.py` 2026-07-15, replacing the retired `5_performance.py`) |
