...
class Predictor:
    def __init__(self, stores=None, progress=None):
        ...
        for side, team, opp, lineup, opp_starter, home in sides:
            ...
            for pid, slot in lineup:
                ...
                row = {"slot": slot, "Home": home, "Season": season,
                       "month": date.month, **b, **st_feats, **toff, **pen,
                       **park, **wx, **env,
    ...
    def _team_rows(self, spec):
        ...
        for team, opp_starter, opp, home in sides:
            ...
            rows.append({
                ...
                "opp_ps_hr_bf": stf["ps_hr_bf"], "opp_pc_era": stf["pc_era"],
    ...
    def predict_game(self, spec):
        ...
        for prop, col in PROP_COLS.items():
            batters[col] = np.round(predict_prop(a["props"][prop], X), 4)
        ...
        sdf, smeta = self._starter_rows(spec)
        if len(sdf):
        ...
        for line in K_LINES:
            starters[f"P_over_{line}"] = [round(poisson_over(l, line), 3)
                                          for l in k_pred]
        ...
        winner, p_win = ((spec["home_team"], p_home) if p_home >= 0.5
                         else (spec["away_team"], 1 - p_home))
    ...
    def predict_slate(self, specs):
        ...
        for spec in specs:
            ...
            games.append({"Game": tag, "Date": spec["date"],
                          "Venue": spec["venue"],
                          "Winner": t["winner"], "WinProb": t["win_prob"],
...
def summary_frame(specs, out):
    ...
    for i, spec in enumerate(specs):
        ...
        for _, r in s.iterrows():
            lines.append((f'{r["Name"]} strikeouts',
                          ...
                          f'{r["P_over_6.5"]:.0%}'))
        lines.append(("Predicted winner",
                      f'{t.get("winner", "?")} wins '
                      f'{t.get("win_prob", 0):.0%} of the time — expected '
                      f'score {spec["away_team"]} {t.get("exp_away_runs", "?")}'
    ...
    if "Game" in b_all.columns and len(specs) > 1:
        top = b_all.nlargest(min(10, len(b_all)), "P_HR")
        lines.append(("Slate note",
                      f"today's top-{len(top)} HR picks come from "
                      f"{top['Game'].nunique()} of {len(specs)} games. Picks "
                      f"from the same game share weather, park, and pitcher - "
                      f"they win and lose together, so avoid stacking too "
                      f"much on one game."))
    ...
    for ws in wb.worksheets:
        ...
        for j, h in enumerate(headers, start=1):
            ...
            for i in range(2, max_row + 1):
                ...
                if h.startswith("P_") or h == "WinProb":
                    c.number_format = "0.0%"
                if i % 2 == 1:
                    c.fill = stripe_fill
...
def save_excel(spec, out, path=None):
    ...
    info_rows = [
        ...
        ("Predicted winner", f'{t.get("winner", "?")} '
                             f'({t.get("win_prob", 0):.0%})'),
        ("Expected score",
...
def _slate_totals(out, i):
    ...
    return {"exp_lineup_HR": g["exp_lineup_HR"],
            ...
            "winner": g["Winner"], "win_prob": g["WinProb"],
...
def selftest(pred):
    ...
    Covers the batter rows and (when the artifact has one) the winner row."""
